...
void CSO_Algorithm(int i_Cycle,int i_Iteration,int i_PSize,int i_Dim,double d_InitL,double d_InitR,double d_MaxVel,bool b_Maximize,int i_FN,mytimer *timer,double d_avefns[],double d_avetime[] , double w[])
{
  ...
  double d_Start, d_Stop;
    ...
    CSO_Initialization(t_cat,i_PSize,i_Dim,d_InitL,d_InitR,d_MaxVel,i_FN); // Initial process of the CSO algorithm
      ...
      CSO_Movement(t_cat,i_PSize,i_Dim,b_Maximize,i_FN,d_MaxVel,w);  // Move the cats in the solution space.
...
int main(int argc, char* argv[])
{
	...
	int i_Iteration[i_FuncNum]={5, 5, 3500}; // Defines the total iteration number in one cycle.
  ...
  double d_InitL[i_FuncNum]={-500, -1 , -32.768};       // Defines the left border of the initial ranges.
  double d_InitR[i_FuncNum]={500, 1 , 32.768};       // Defines the right border of the initial ranges.
  ...
  timer=mytimer_create();
    ...
    /* Initialize the parameters for recording the results - End   */
		w = new double[i_Dim[(i-1)]];
              for(int k = 0 ; k < i_Dim[(k-1)] ; k++){
			w[k] = unifRand();
			if(w[k] < 0.4)
				w[k] += 0.4;
		}

		CSO_Algorithm(i_Cycle[(i-1)],i_Iteration[(i-1)],i_PSize[(i-1)],i_Dim[(i-1)],d_InitL[(i-1)],d_InitR[(i-1)],d_MaxVel[(i-1)],b_Maximize[(i-1)],i,timer,d_avefns,d_avetime,w);  // Call the CSO algorithm to solve the problem.
